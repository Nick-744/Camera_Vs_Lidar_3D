...
def point_cloud_from_disparity(disparity, calib, left_color):
    f, cx, cy, Tx = calib['f'], calib['cx'], calib['cy'], calib['Tx']
    h, w = disparity.shape
    mask = disparity > 0
...
def main():
    ...
    for idx in range(10, 30):
        ...
        color_overlay = left_color.copy()
        color_overlay[mask == 255] = [255, 0, 0]  # mark road in blue

        #cv2.imshow("Detected Road Mask", mask)
        cv2.imshow("Overlay", color_overlay)
        cv2.waitKey(0)
